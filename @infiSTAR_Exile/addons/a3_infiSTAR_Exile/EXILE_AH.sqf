...
FNC_A3_LOG_RAND_VAR = compileFinal "
	'ARMA_LOG' callExtension format['A3_RANDOMVAR_LOG:%1 (v0010)',_this];
	diag_log ('<infiSTAR.de>RandomVarLog| '+_this+' (v0010)');
...
FNC_A3_HACKLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_HACKLOG:%1 [FPS: %2|THREADS: %3] (v0010)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>HL| '+_this+' (v0010)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_HACKLOG'] call MAR_fnc_log;};
";
FNC_A3_SURVEILLANCELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_SURVEILLANCELOG:%1 [FPS: %2|THREADS: %3] (v0010)',_this,diag_fps,count diag_activeSQFScripts];
	diag_log ('<infiSTAR.de>SL| '+_this+' (v0010)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de_SURVEILLANCELOG'] call MAR_fnc_log;};
";
FNC_A3_ADMINLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_ADMINLOG:%1 (v0010)',_this];
	diag_log ('<infiSTAR.de>AdminLog| '+_this+' (v0010)');
	if(!isNil'MAR_fnc_log')then{[_this,'infiSTAR.de__ADMINLOG'] call MAR_fnc_log;};
";
FNC_A3_CONNECTLOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_CONNECTLOG:%1 (v0010)',_this];
	diag_log ('<infiSTAR.de>ConnectLog| '+_this+' (v0010)');
";
FNC_A3_RESOLVESTEAMNAMELOG = compileFinal "
	'ARMA_LOG' callExtension format['A3_RESOLVESTEAMNAMELOG:%1 (v0010)',_this];
	diag_log ('<infiSTAR.de>ResolveSteamName| '+_this+' (v0010)');
...
fnc_call_ARMALOAD = compileFinal "
	...
	if(_option == 0)exitWith
	{
		...
		if(count _input == 3)then
		{
			...
			[[_name,_puid,_res],_code,_clientID] call FN_infiSTAR_S;
...
_A3AHstring = "
	...
	"+_fnc_pvc_token+" = compileFinal ""
		...
		[[_clientName,_clientUID],"+_AH_MAIN_BLOCK+"] remoteExec ['FN_infiSTAR_C', _clientID];
	...
	FNC_INFISERVERKICK = compileFinal ""
		...
		diag_log format['<infiSTAR.de>KickLog| %1(%2)   (v0010)',_name,_uid];
	...
	FNC_A3_infiSTARBAN = {
		...
		if(_reason == '')then
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) (v0010)',_name,_uid];
			diag_log format['<infiSTAR.de>BanLog| %1(%2)   (v0010)',_name,_uid];
		}
		else
		{
			'ARMA_LOG' callExtension format['A3_BANLOG:%1(%2) %3 (v0010)',_name,_uid,_reason];
			diag_log format['<infiSTAR.de>BanLog| %1(%2) %3 (v0010)',_name,_uid,_reason];
		};
	...
	_FNC_PVAH_AdminReq = {
		...
		if(isNil '_clientNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _clientNetID != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq clientNetID wrong typeName: %1   %2 (v0010)',typeName _clientNetID,_clientNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_clientNetID == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq clientNetID is empty (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		_ObjFromNetID = objectFromNetId _clientNetID;
		if(isNil '_ObjFromNetID')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _ObjFromNetID != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq ObjFromNetID wrong typeName: %1   %2 (v0010)',typeName _ObjFromNetID,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _ObjFromNetID)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq ObjFromNetID is Null (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_array')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _array != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq wrong typeName: %1   %2 (v0010)',typeName _array,_array];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_option')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq option is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _option != 'SCALAR')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq option wrong typeName: %1   %2 (v0010)',typeName _option,_option];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _playerObj != 'OBJECT')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj wrong typeName: %1   %2 (v0010)',typeName _playerObj,_playerObj];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(isNull _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq playerObj is Null (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if!(_ObjFromNetID isEqualTo _playerObj)then
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq playerObj != _ObjFromNetID - %1/2  (v0010)',_playerObj,_ObjFromNetID];
			_log call FNC_A3_SURVEILLANCELOG;
		...
		if(!isPlayer _playerObj)then
		{
			if(_option == 1234)then
			{
				...
				if(isNil '_puid')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + 'AdminReq puid is Nil (v0010)';
					_log call FNC_A3_SURVEILLANCELOG;
				};
				if(typeName _puid != 'STRING')exitWith{
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['AdminReq puid wrong typeName: %1   %2 (v0010)',typeName _puid,_puid];
					_log call FNC_A3_SURVEILLANCELOG;
				};
				
				_mytime = call fnc_getserverTime;
				_log = format['AdminReq playerObj is not a player   %1(%2 - %3) - trying to resolve player via allPlayers (v0010)',name _playerObj,getPlayerUID _playerObj,_puid];
				_log = _mytime + _log;
				_log call FNC_A3_SURVEILLANCELOG;
				if(_puid == '')exitWith{};
				{
		...
		if(!isPlayer _playerObj)exitWith{
			_mytime = call fnc_getserverTime;
			_log = format['AdminReq playerObj is not a player   %1(%2) (v0010)',name _playerObj,getPlayerUID _playerObj];
			_log = _mytime + _log;
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['AdminReq tokenreceived wrong typeName: %1   %2 (v0010)',typeName _tokenreceived,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(_tokenreceived == '')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'AdminReq tokenreceived is empty (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_clientUID isEqualTo '')then
		{
			...
			if(_foundtokenid > 0)then
			{
				...
				if(!isNil '_puidfound')then
				{
					...
					_mytime = call fnc_getserverTime;
					_log = _mytime + format['%1(%2) | AdminReq - SERVER RESOLVED UID! (v0010)',_clientName,_clientUID];
					_log call FNC_A3_SURVEILLANCELOG;
				};
			};
		};
		...
		if(!_result)exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - BadToken %3 (v0010)',_clientName,_clientUID,_tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		_fnc_NoUID = {
			...
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - !!!!!!ADMIN-LOGIN-ERROR!!!!!! (v0010)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		call {
		...
		{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['%1(%2) | AdminReq - Attempted to Use the AdminMenu (v0010)',_clientName,_clientUID];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(_option == 1234)exitWith
		{
			...
			if(_puid == _clientUID)then
			{
				if!(_clientUID in _locDevs)then{
					diag_log format['<infiSTAR.de> ******ADMIN-LOGIN******: %1(%2) (v0010)',_clientName,_clientUID];
				};
				"; if(_enableIngameLogs)then{ _A3AHstring = _A3AHstring + "
				...
				if(isNil'infiSTAR_MAIN_CODE')exitWith{
					...
					['',_code,_clientID] call FN_infiSTAR_S;
				};
				['',infiSTAR_MAIN_CODE,_clientID] call FN_infiSTAR_S;
			};
		};
		if(_option == 69)exitWith
		{
			if(_clientUID in _allPlayers)then
			{
				...
				if(_playerObj isEqualTo objectFromNetId _netId)then
				{
					...
					_code = compile _code;
					if(_opt == 0)exitWith{call _code;['',_code,-2] call FN_infiSTAR_S;};
					...
					if(_opt == 2)exitWith{
						...
						['',_code,_owner] call FN_infiSTAR_S;
					};
				};
			};
		};
		...
		if(_option == -664)exitWith
		{
			...
			['',{(findDisplay 46)closeDisplay 0;},(owner _target)] call FN_infiSTAR_S;
		};
		...
		if(_option == 0)exitWith
		{
			...
			_code = {
				_containerObject = objectFromNetId _this;
				player moveInDriver _containerObject;
			};
			[_netIdvehObj,_code,(owner _target)] call FN_infiSTAR_S;
		};
		...
		if(_option == 9)exitWith
		{
			...
			if(isPlayer _target)then
			{
				...
				if(typeOf _target isEqualTo 'Exile_Unit_Player')then
				{
					...
					['',_code,(owner _target)] call FN_infiSTAR_S;
				}
				else
				{
		...
		if(_option == 13)exitWith
		{
			...
			[_value,_code,(owner _target)] call FN_infiSTAR_S;
		};
		...
		if(_option == 5000)exitWith
		{
			...
			{
				[_txt,_code,_targetID] call FN_infiSTAR_S;
			};
			[_txt,_code,_clientID] call FN_infiSTAR_S;
		};
		if(_option == 9876)exitWith
		{
			...
			if(_steamName != '')then
			{
				...
				[[_name,_puid,_steamname],_code,_clientID] call FN_infiSTAR_S;
	...
	_FNC_AH_KICKLOGSPAWN =
	{
		...
		if(isNil '_input')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + 'FNC_AH_KICKLOGSPAWN _this is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_tokenreceived')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_tokenreceived _this is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _tokenreceived != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_tokenreceived wrong type %1 (v0010)',typeName _tokenreceived];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(isNil '_arraysent')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + '_arraysent _this is Nil (v0010)';
			_log call FNC_A3_SURVEILLANCELOG;
		};
		if(typeName _arraysent != 'ARRAY')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_arraysent wrong type %1 (v0010)',typeName _arraysent];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if(typeName _netId != 'STRING')exitWith{
			_mytime = call fnc_getserverTime;
			_log = _mytime + format['_netId wrong type %1 (v0010)',typeName _netId];
			_log call FNC_A3_SURVEILLANCELOG;
		};
		...
		if!(_puid isEqualTo _puidfound)then
		{
			_puid = _puidfound;
			_arraysent set[1,_puid];
		};
	...
	fnc_serverMassMessage = compileFinal ""
		...
		[_msg,_code,_clientID] call FN_infiSTAR_S;
	"";
...
"; }; _A3AHstring = _A3AHstring + "
	...
	_devLog = format['%1 VERSION: 28092015 infiSTAR.de AHAT (v0010) - server running: %2:1',VERSIONCHECKRESULT,call _GET_TIME_TIME];diag_log _devLog;
	"; if(!_HIDE_FROM_PLAYERS)then{ _A3AHstring = _A3AHstring + "
		...
		if(_puid in "+_adminsA+")then
		{
			...
			if(isNil'fnc_AdminReq')then
			{
				fnc_AdminReq = compileFinal ""
					...
					["+_YourPlayerToken+",_this,netId player] remoteExecCall ['fnc_AdminReq', 2];
				"";
			};
		...
		_fnc_zero_two =
		{
			if(time > _mtimer)then
			{
				...
				"; if(_CLM)then{ _A3AHstring = _A3AHstring + "
					...
					[[_admins,allMapMarkers],_code] remoteExec ['FN_infiSTAR_C', -2];
				"; }; _A3AHstring = _A3AHstring + "
	...
	if(hasInterFace)then
	{
		...
		while{1==1}do
		{
			...
			if(time > _timer0)then
			{
				...
				['',_DO_THIS_MORE_OFTEN,-2] call FN_infiSTAR_S;
				uiSleep 1;
						...
						if(alive _x)then
						{
							...
							if!(_uid isEqualTo '')then
							{
								...
								if(_serverMoney > 0)then
								{
									...
									[_serverMoney,_code,owner _x] call FN_infiSTAR_S;
								};
